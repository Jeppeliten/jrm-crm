...
class CRMApplication {
  ...
  setupLifecycleHandlers() {
    ...
    window.addEventListener('online', () => {
      ...
      this.components.get('notifications')?.show('Anslutning återställd', 'success');
    });
    ...
    try {
      ...
      if (!validationResult.isValid) {
        ...
        this.components.get('notifications')?.show(
          'Dataintegritetsvarningar upptäckta. Se konsolen för detaljer.',
          'warning'
        );
      }
      ...
      try {
        ...
        this.components.get('notifications')?.show('Data återställd från backup', 'info');
      } catch (backupError) {
        console.error('Backup restoration failed:', backupError);
        
        // Last resort: seed with fresh data
        await this.seedInitialData();
        this.components.get('notifications')?.show('Ny datauppsättning skapad', 'info');
      }
    }
  }
  ...
  getUserFriendlyErrorMessage(error, type) {
    const messages = {
      ...
      'VALIDATION_ERROR': 'Ogiltiga data upptäckta. Kontrollera dina inmatningar.',
      'SECURITY_VIOLATION': 'Säkerhetsvarning upptäckt. Kontakta administratören.',
      'UNKNOWN_ERROR': 'Ett oväntat fel inträffade. Försök igen.'
    };
  ...
  handleBeforeUnload(event) {
    ...
    if (state?.hasUnsavedChanges()) {
      const message = 'Du har osparade ändringar. Är du säker på att du vill lämna sidan?';
   ...
   */
  async shutdown() {
    console.log('🔄 Shutting down CRM Application...');
